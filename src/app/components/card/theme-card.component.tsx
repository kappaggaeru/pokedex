...
export const ThemeCardComponent: React.FC<ThemeCardProps> = ({ title, icon: Icon, enabled }) => {
    ...
    return (
        ...
        ${enabled && !checked ? "hover:bg-slate-100 dark:hover:bg-slate-700 " : "bg-slate-50"}
        ${enabled && checked ? "border-indigo-300 dark:border-indigo-500" : ""}
        ${enabled && checked ? "bg-indigo-100 dark:bg-slate-800/20" : "bg-white dark:bg-slate-800/20"}
        `}
